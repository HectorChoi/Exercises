...
# algorithm:
# create a empty array to add non-duplicates / check for duplicates
# for each element in the duplicate containing array
#   check to see if it is included in the helper array
#   if so, return the duplicate number
#   if not, add it to the helper array

def find_dup(arr)
  helper = []
  arr.each do |num|
    return num if helper.include?(num)
    helper << num
  end
